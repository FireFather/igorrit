#ifndef BUILD_nodo_totale
#define BUILD_nodo_totale
#include "RobboLito.h"
#include "storia.i"
#include "mossa_nullo.i"

#include "nodo_totale.c"
#include "bianco.h"
#else
#include "nero.h"
#endif
...
int
mio_nodo_totale (tipo_posizione *POSIZIONE, int VALU, int abisso)
{
  int mossa, i;
  int k;
  tipo_zobrist *tr;
	  ...
	  if (tr->abisso_alto >= abisso)
	    {
	      ...
	      if (valu < VALU)
		{
		  if (!((tr->pietras & pietra_tagliata) == pietra_tagliata))
		    {
		      UPDATE_ANNI();
		      RETURN (valu);
		    }
  ...
  if (POSIZIONE0->valu >= VALU && mio_nullo)
	...
	{
	  if (mossa_tras == 0)
	    Zobrist_basso_totale (POSIZIONE, 0, abisso, v);
	  RETURN (v);
	}
    }
  ...
  while (VERO)
      ...
      if (ripetizione_interrogativo (0))
	{
	  ...
	  && abisso < 20)
	{
	  if ((5 << (abisso - 6)) + MASSIMO_POSIZIONALE (mossa) +
      ...
      else
	{
	  ...
	  if (PROSSIMO->fase == mossa_ordinaria && cnt >= 3)
	    {
	      abisso_nuovo = abisso - 2 + ESTENSIONE - BSR (1 + cnt);
	      if (abisso_nuovo <= 1)
		v = -tuo_cerca_fine (POSIZIONE, 1 - VALU, 0);
	      else if (abisso_nuovo <= 7)
		v = -tuo_cerca_mezzo (POSIZIONE, 1 - VALU, abisso_nuovo);
      ...
      if (v >= VALU)
	{
	  ...
	  Zobrist_basso_totale (POSIZIONE, mossa, abisso, v);
	  RETURN (v);
	}
      if ((POSIZIONE0 + 1)->cattura == 0 && mossa_storia (mossa))
  ...
  if (!cnt && PROSSIMO->fase <= TRAS2)
    RETURN (0);
  v = VALU - 1;
  Zobrist_alto (DINAMICO->Zobrist, abisso, v);
  RETURN (v);
}

int
mio_nodo_totale_scacco (tipo_posizione *POSIZIONE, int VALU, int abisso)
{
  ...
  while (p->mossa || !GEN)
      ...
      if (v < VALU)
	{
      ...
      Zobrist_basso_totale (POSIZIONE, mossa, MASSIMO (1, abisso), v);
      RETURN (v);
    }
